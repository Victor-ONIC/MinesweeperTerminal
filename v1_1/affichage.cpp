 ...
 */
void func_clear(void) { cout << "\x1B[2J\x1B[3J\x1B[H"; }

/**
 * Affiche une interface afin que le joueur choisisse les caractéristiques
 * du jeu.
 *
...
void menu(int &ligne, int &colonne, int &difficulty) {  // OK
    int taille = 0;
    int choix_diff = 0;
    while ((taille == 1 && choix_diff > 2) ||
           (taille == 2 && choix_diff > 6) || 
           taille <= 0 || taille > 4 ||
           choix_diff <= 0 || choix_diff > 8) {
        func_clear();
...
void do_what(matrice *T, int &action, int &i, int &j, bool is_init) {
    ...
    if (is_init) {
        cout << "\nOu creuser ? ( par exemple -> 4 (ligne 4), -> 6 "
                "(colonne 6) )";
        do {
            cout << "\n-> ";
            cin >> i;
 ...
 * @param lost indique s'il faut afficher les mines quand le joueuer a
 * perdu.
 */
...
void aff_banner() {
    ...
    for (int i = 0; i < banner.size(); i++) cout << banner[i] << "\n";
