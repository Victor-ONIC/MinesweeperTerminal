...
bool condition_init_jeu(matrice *T, int mine_ligne, int mine_colonne,
  ...
  return (
    ...
    ((mine_ligne >= ask_ligne - 1 && mine_ligne <= ask_ligne + 1 ) &&
    (mine_colonne >= ask_colonne - 1 && mine_colonne <= ask_colonne + 1))
  );
}

/**
 * Initialise la matrice du jeu avec des mines aléatoires après la première
 * creusée.
 * 
...
matrice *Init_jeu(int nb_ligne, int nb_colonne, int difficulty, 
  ...
  for (int i = 0; i < difficulty; i++) {  // placer les mines
    ...
    while (condition_init_jeu(T, mine_ligne, mine_colonne, ask_ligne, ask_colonne)) {
  ...
  for (int ligne = 0; ligne < M->nbl; ligne++) {
    for (int colonne = 0; colonne < M->nbc; colonne++) {
      if (T->T[ligne][colonne] != 2) {
        ...
        for (int i = (ligne - 1 + is_lig_zero); i <= (ligne + 1 - is_lig_max);
             i++) {
...
/**
 * Creuse une case, puis toutes les cases adjacentes qui ne contiennement pas de
 * mines. Utilisée en complément avec creuser_recu.
 * 
 * @param T matrice du jeu.
...
int creuser(matrice *T, matrice *M, int i, int j, int &N) {
  if (!(T->T[i][j] == 0 || T->T[i][j] == 2)) {  // ne peut pas être creusée
    return -1;
  }
...
void creuser_recu(matrice *T, matrice *M, int ligne, int colonne, int &N) {
  ...
  if (M->T[ligne][colonne] == 0) {
    int is_lig_zero = ligne == 0 ? 1 : 0;
    int is_col_zero = colonne == 0 ? 1 : 0;
    int is_lig_max = ligne == (T->nbl - 1) ? 1 : 0;
    int is_col_max = colonne == (T->nbc - 1) ? 1 : 0;

    for (int m = (ligne - 1 + is_lig_zero); m <= (ligne + 1 - is_lig_max);
         m++) {
      for (int n = (colonne - 1 + is_col_zero); n <= (colonne + 1 - is_col_max);
           n++) {
